...
const initETHData = { name: "Ethereum", symbol: "ETH", price: 0, daily_percent: 0 };
...
function Wallet() {
  ...
  const [ethData, setEthData] = useState(initETHData);
  const wallet = new WalletUtil()
  const serverUrl = SERVER_URL;
  let frequent, ethFrequent;
  ...
  const getEthPrice = async () => {
    axios.get(`${SERVER_URL}wallets/getsingletokendata/ETH`, {
    })
      .then((response) => {
        if (response.data.response) {
          setEthData(response.data.data);
        }
      })
      .catch(err => {
        console.log(err);
        setEthData(initETHData)
      })
  }
...
>>>>>>> origin/add-wallet-change-to-profile-page
  ...
  useEffect(() => {
    getEthPrice();
    ethFrequent = setInterval(getEthPrice, 1000 * 60);
    return () => {
      clearInterval(ethFrequent);
    }
  }, [])
              ...
              <Col xs={{ span: 24 }} md={{ span: 6 }} className="bg-white border-l-8 border-gray-200 myColor1">
                ...
                <Row className="border-b-8 border-gray-200 p-4 text-center">
                  ...
                  <Col span={24} className="text-overflow">
                    <PairCardButton key={0} title={ethData.symbol} amount1={ethData.price} percent={ethData.daily_percent} />
                  </Col>
                </Row>
                <Row className="p-4 ">
                  <Col span={24} className="text-overflow">
